...
from data_loader.ffhq import BOX
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.Sequential(EqualLinear(4608, feats_dim, activation="fused_lrelu"),
    ...
    def forward(self, img, mask):
        ...
        del out
        face = self.stem2(self.stem(img * mask))
        return face, nodes
...
class Generator(nn.Module):
    def __init__(
        ...
        self.conv1 = StyledConv(
            self.channels[in_fs], self.channels[in_fs], 3, style_dim, blur_kernel=blur_kernel, box=box
        )
        ...
        for i in range(start + 1, self.log_size + 1):
            ...
            self.convs.append(
                StyledConv(
                    out_channel, out_channel, 3, style_dim, blur_kernel=blur_kernel, box=box
                )
            )
